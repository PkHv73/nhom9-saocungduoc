...
AGENT1_SYSTEM = """Bạn là Agent Phân loại Ticket của hệ thống ZaloPay.
Nhiệm vụ: Đọc nội dung ticket khách hàng, xác định nhóm nghiệp vụ và trích xuất thông tin quan trọng.
Chỉ trả về JSON hợp lệ, không giải thích, không thêm bất kỳ văn bản nào khác.
...
Nhận thông tin phân loại ticket và kết quả kiểm tra hệ thống, phân tích nguyên nhân và hướng xử lý.
Chỉ trả về JSON hợp lệ, không giải thích, không thêm văn bản nào khác.
Cấu trúc JSON bắt buộc:
{
...
def agent1_classify(client: OpenAI, ticket: str) -> TicketClassification:
    """Agent 1: Phân loại ticket và trích xuất thông tin."""
    log.info("[Agent 1] Đang phân loại ticket...")
    prompt = f"Ticket khách hàng:\n{ticket}"
    raw = call_claude(client, AGENT1_SYSTEM, prompt)
...
def agent2_analyze(
    ...
    """Agent 2: Phân tích nghiệp vụ dựa trên phân loại và kết quả kiểm tra."""
    log.info("[Agent 2] Đang phân tích nghiệp vụ...")
    prompt = (
        f"Thông tin phân loại ticket:\n{json.dumps(classification.__dict__, ensure_ascii=False, indent=2)}\n\n"
        f"Kết quả kiểm tra hệ thống:\n{check_result or '(Chưa có kết quả kiểm tra)'}"
    )
    raw = call_claude(client, AGENT2_SYSTEM, prompt)
    log.debug("[Agent 2] Raw output:\n%s", raw)
    ...
    log.info(
        "[Agent 2] ✓ refund_allowed=%s | escalate=%s",
        result.refund_allowed,
        result.escalate,
    )
    return result
...
def run_pipeline(ticket: str, check_result: str = "") -> PipelineResult:
    ...
    try:
        ...
        # Bước 0 (gộp soạn thảo + chuẩn hóa)
